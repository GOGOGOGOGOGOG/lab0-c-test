...
void report_event(message_t msg, char *fmt, ...) {
  ...
  char *msg_name =
      msg == MSG_WARN ? "WARNING" : msg == MSG_ERROR ? "ERROR" : "FATAL ERROR";
...
void mem_status(FILE *fp) {
  fprintf(fp, "Allocated cnt/bytes: %lu/%lu.  Freed cnt/bytes: %lu/%lu.\n"
...
void init_time(double *timep) { (void)delta_time(timep); }
...
double gigabytes(size_t n) { return (double)n / (1UL << 30); }

void reset_peak_bytes() { last_peak_bytes = current_bytes; }
